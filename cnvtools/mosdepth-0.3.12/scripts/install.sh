...
apt-get update
apt-get -qy install bwa make build-essential cmake libncurses-dev ncurses-dev libbz2-dev lzma-dev liblzma-dev \
...
apt-get update
apt-get install git llvm curl wget libcurl4-openssl-dev
...
cp ../d4-format/target/release/libd4binding.* /usr/local/lib
cp ./d4binding/include/d4.h /usr/local/include/
ldconfig
